...
class SimpleMap extends Component {
  ...
  getMaterial = async topic => {
    const result = await API.getMaterial(topic);
    const results = await result.data;
      console.log("BACK FROM GET MATERIAL: ", results);
      const materialIds = await results.result.map((obj) => obj.material_id);
      console.log(materialIds);
      return materialIds    
  }
  getRecycleLocations = async (materialIds, latitude, longitude) => {
      // API.getID(materialIds, this.state.center.lat, this.state.center.lng).then(function (materialLocations) {
    // let saveFunction = this.saveLoc;    
    const materialLocation = await API.getID(materialIds, latitude, longitude);
    const materialLocations = await materialLocation.data.result;
    console.log("back from getID: ", materialLocations);
    const recycleLocations = await materialLocations.map((location, index) => {
          console.log(location);
          return [
            location.description,
            location.latitude,
    ...
    this.setState({recycleLocations: recycleLocations})      
        // saveFunction(recycleLocations);
        // initMap(recycleLocations)
  }
  ...
  handleFormSubmit = async (event) => {
    event.preventDefault();
    console.log(this.state);
    const materialID = await this.getMaterial(this.state.topic, this.state.center.lat, this.state.center.lng);
    await this.getRecycleLocations(materialID, this.state.center.lat, this.state.center.lng);
  };
